...
const skicamsReq = new XMLHttpRequest();
skicamsReq.open('GET', 'https://makevoid-skicams.p.mashape.com/cams.json');
skicamsReq.setRequestHeader("X-Mashape-Key", "kxSXmUymofmshFHhhKxWOSJpqJsJp1I3zNnjsnqKwhITAiC1zw");
skicamsReq.send(null);
skicamsReq.onreadystatechange = function () {
    if (skicamsReq.readyState == 4) {

        const video = JSON.parse(skicamsReq.response)

        image1.src = video['9'].cams['44'].url;
        image2.src = video['9'].cams['44'].url;
        image3.src = video['9'].cams['45'].url;
        image4.src = video['9'].cams['45'].url;
